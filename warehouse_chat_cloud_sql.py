"""
WAREHOUSE PUTAWAY RECOMMENDATION SYSTEM - CLOUD SQL VERSION

Simple interactive system that answers: "Where should I put Part X?"

Uses:
- Qdrant for historical patterns
- Cloud SQL (MySQL) for real-time location availability

This is the Cloud SQL production version.
For local development, use warehouse_chat.py
"""
...
# OPTION 1: Direct connection (requires whitelisted IP)
db = mysql.connector.connect(
    host='35.198.187.177',  # Your Cloud SQL IP
    port=3306,
...
# OPTION 2: Using Cloud SQL Proxy (recommended for security)
# Start proxy first: cloud_sql_proxy.exe -instances=PROJECT:REGION:warehouse-prod=tcp:3307
# Then use:
# db = mysql.connector.connect(
#     host='127.0.0.1',
...
# OPTION 3: With SSL/TLS (most secure)
# db = mysql.connector.connect(
...
def recommend_location(part_id):
    ...
    # STEP 2: AUGMENTATION - Filter valid locations and check Cloud SQL status
    ...
    for loc in locations:
        ...
        # Get current status from Cloud SQL
...
def chat():
    ...
    print("="*80)
    print(" " * 20 + "WAREHOUSE PUTAWAY ASSISTANT (CLOUD SQL)")
    print("="*80)
    print()
