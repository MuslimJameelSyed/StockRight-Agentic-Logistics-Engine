...
def generate_report(df, pdf_path, charts_dir):
    ...
    pdf.cell(0, 6, "Accuracy by Pattern Strength", new_x="LMARGIN", new_y="NEXT")
    pdf.image(strength_img, w=pdf.epw); pdf.ln(5)
    ...
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 6, "Accuracy by Confidence Score", new_x="LMARGIN", new_y="NEXT")
    pdf.image(conf_img, w=pdf.epw); pdf.ln(5)

    # --- verification table (ALL 100 parts) ---
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 6, "Complete Verification Table (All 100 Parts)", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)
